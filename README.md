...
| 📚 **知识库 RAG** | 11 份规范规则（GB16297、GB12523-2025、DB61/1078、GBZ2.1/2.2、安监总安健〔2012〕89 号等）；启动时压缩为 “规则精要” 注入，命中隐患后再按需召回全文；支持 GitHub raw 实时同步，失败自动降级本地 |
...
1. 将 `knowledge/` 目录推到团队仓库（如 `https://github.com/team/repo/tree/main/knowledge`）；

2. 侧边栏选择 “GitHub 实时同步”，填入 raw 目录地址：

   `https://raw.githubusercontent.com/team/repo/main/knowledge`；

3. 点 “从 GitHub 同步”：11 份文件全部成功即用云端版本；任一失败自动保留本地库，不影响演示。

   新增规范只需按 `序号-名称.md` 命名上传，并在 `core/config.py` 的 `HAZARD_CATALOG` 增加分类映射。
